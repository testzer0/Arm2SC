...
public:
	...
	// members for the random_exec helper
	vector<struct event *> *events;		// set of _all_ events, sorted by fetch time	
	bool allfetched = false;			// have we fetched all instructions yet?
	uint32_t *regs_to_fill;				// this is where we should write the final reg values
	double fetch_prob;					// probability of fetching when taking a random transition
	...
	void random_exec_helper();

	// Random execution
	void random_execution_init(struct random_execution_args *args);
