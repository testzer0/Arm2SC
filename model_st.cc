...
process::process(proc_t pid) : proc_id(pid)
{
	...
	events = new vector<struct event *>;
...
/**
 * random_execution_init - Initializes all structures needed for
 * the random execution.
 */
void process::random_execution_init(struct random_execution_args *args)
{
	regs_to_fill = args->regs_to_fill;
	fetch_prob = args->fetch_prob;
...
/**
 * random_exec_helper - takes a random transition in this process, and does the bookeeping
 * tasks. 
 */
void process::random_exec_helper()
{
	if (finished)
		return;
	struct event *e;
	...
	if (active_events.size() && (allfetched || (double)random(1,INT_MAX) > (fetch_prob*INT_MAX))) {
		...
		take_next_possible_transition(active_events[ind]);
	} else {
...
/**
 * do_random_exec - performs a random execution. The advantage of this over the
 * multithreaded variant is that it allows a process to simply "freeze" for a long time, 
 * e.g. if a process executes instructions 2,3,5 then does nothing whil another process
 * executes 10 instructions, after which the former executes 1: such behaviour is unlikely
 * with many threads as each thread keeps making progress: the st version is hence slower
 * but more exhaustive.
 */
void do_random_exec()
{
	while (activeprocs.size()) {
		int ind = random(0,activeprocs.size()-1);
		processes[activeprocs[ind]].random_exec_helper();
		if (processes[activeprocs[ind]].finished) {
			if (processes[activeprocs[ind]].have_taken_wrong_branch)
				return;
			activeprocs.erase(activeprocs.begin()+ind);
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < nproc; i++) {
		...
		processes[i].random_execution_init(args);
	}
