...
uint32_t process::init_nonacq_read(struct event *e)
{
	...
	e->status = INIT;
	#ifdef DEBUG
		cout << proc_id << " " << e->instr->instr_id << ' ' <<
		 e->ins_type << " LD RESULT " << e->result << endl;
	#endif
	return e->event_num;
}
...
uint32_t process::init_st(struct event *e)
{
	...
	#ifdef DEBUG
		cout << proc_id << " " << e->instr->instr_id << " ST RESULT " << e->result << endl;
	#endif
...
uint32_t process::com_aci(struct event *e)
{
	...
	if (!aci_com_cnd(e)) {
		...
		if (have_taken_wrong_branch) {
			std::cerr << "Taken wrong branch at event_num: " << e->event_num << std::endl;
			exit(-1);
		}
		return 0xFFFFFFFF;
	}
...
uint32_t process::init_assign(struct event *e)
{
	...
	e->result = evaluate_exp(e->instr->operand2, regvals);
	#ifdef DEBUG
		cout << proc_id << " " << e->instr->instr_id << ' ' << " ASSIGN RESULT " << e->result << endl;
	#endif
...
void process::read_in_code(const char *filename)
{
	...
	while (!seenterm) {
		...
		if (last) 											// for now we don't handle ACI so always left
			last->childl = cur;
		else {
			ins_tree_root = cur;
			max_i[0] = cur;
		...
		if (t1 == "LD") {
			...
			// Not handled yet. Besides, it is unlikely that we will directly translate
			// BEQs, BNEs etc to ACI _before_ parsing it here; hence we most likely will
			// be parsing BEQs and BNEs etc instead
		} else if (t1 == "TERM") {
			cur->operand1 = NULL;
			cur->operand2 = NULL;
			cur->ins_type = TERM;
...
inline event *read_global_memory(uint32_t index, uint32_t proc_id)
{
	...
	#ifdef DEBUG
		cout << proc_id << " READING " << global_memory[index] << " FROM " << index << endl;
	#endif
